...
function concat(filename) {
  if (!read[filename]) {
    ...
    var bundled = utils.readToken($, dir);
    if (bundled) {
      for (var i = 0; i < bundled.length; i++) {
        read[bundled[i]] = true;
      }
      utils.removeToken($);
      // reparent the body content from the vulcanize hidden div
      var hiddenEl = $('body > div[hidden]');
      hiddenEl.replaceWith(hiddenEl.html());
    }
    processImports($);
...
function handleMainDocument() {
  ...
  if (options.bundle) {
    var token = utils.makeToken(read, dir);
    $('head').append(token);
  }
