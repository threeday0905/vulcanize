...
module.exports = {
  ...
  makeToken: function(cache) {
    var files = Object.keys(cache);
    return '<meta name="vulcanize" content="' + files.join() + '">';
  },
  readToken: function($) {
    var tokenEl = $('meta[name="vulcanize"]');
    if (!tokenEl.length) {
      return [];
    }
    var str = tokenEl.attr('content');
    return str.split(',');
  },
  removeToken: function($) {
    $('meta[name="vulcanize"]').remove();
  }
};
